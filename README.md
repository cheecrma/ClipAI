...
> 기본 모델은 **Gemma 2 2B** (Gemma 약관 — **상업/회사 업무 사용 가능**) 입니다.
> 한국어 품질을 최우선하는 **개인/연구용**은 EXAONE 3.5(비상업, NC)로 교체 가능 → [§11 라이선스](#11-라이선스--사용-제한).
...
- **기본: Gemma 2 2B Instruct** (Google), `Q4_K_M` GGUF (`models/gemma-2-2b-it-Q4_K_M.gguf`, ~1.6GB).
  - **Gemma 약관상 상업 사용 가능** → 회사 업무용에 적합 (메일 요약/번역 등).
  - 한국어도 무난하지만, EXAONE 대비 한국어가 약간 어색하고 지시 따르기가 덜 정교함
    (그래서 한국어 출력 모드의 프롬프트에 "반드시 한국어로" 를 명시해 보정).
- **개인/연구용 대안: EXAONE 3.5 2.4B** (LG AI Research) — 한국어 품질 최상이지만 **비상업(NC)**.
  `models/` 에 넣고 `config.toml` 의 `path` 를 바꾸면 사용.
- **모델 교체 자유**: `models/` 에 원하는 `.gguf` 를 넣으면 됨 → [models/MODEL_GUIDE.md](models/MODEL_GUIDE.md).
  - 엔진은 system 프롬프트를 user 메시지에 합쳐 보내, **Gemma/EXAONE/Qwen 등 어떤 모델이든 호환**.
- **GGUF** = llama.cpp 용 단일 파일 모델 포맷 (가중치 + 토크나이저 + 채팅 템플릿 + 메타데이터).
  `Q4_K_M` 은 품질 손실이 거의 없는 표준 양자화 수준.
...
│   ├── engine.py             # LLM 엔진 (llama-cpp 인프로세스), 스트리밍, 입력 길이 맞춤(fit_input)
...
### 6.1 추론 백엔드: llama-cpp-python (인프로세스)
- 개발 초기엔 이미 설치돼 있던 **Ollama**로 빠르게 프로토타입했으나,
  "Ollama 불필요 단독 exe" 목표 달성 후 **Ollama 백엔드 코드는 제거**했다.
- 현재는 **llama-cpp-python 인프로세스 백엔드만** 지원 (`backend="llamacpp"`).
  모델은 시작 시 1회 로드 후 상주, 토큰 스트리밍.
...
```toml
[model]
backend   = "llamacpp"   # 인프로세스 실행. 현재 이 백엔드만 지원
...
### 모델 (가장 중요)
- **기본 모델 = Gemma 2 2B** : **Gemma 약관(Gemma Terms of Use)** 으로 **상업 사용 허용**
  (단 금지 사용 정책 준수 필요). 회사 업무용에 쓸 수 있습니다. — 약관은 직접 확인 권장.
- **EXAONE 3.5(대안)** : **"EXAONE AI Model License Agreement 1.1 - NC"** = **비상업(NC)**.
  연구·교육·개인·내부 검토만 허용, **상업/회사 업무 사용 금지**. 개인 한국어 용도로만 쓰세요.
- 그 외 상업 가능 예: **Qwen2.5 (Apache-2.0: 1.5B/7B 등)**, **Meta Llama 3.x**(Llama 약관) — 각 라이선스 직접 확인.
- 이 저장소에는 **모델 가중치를 포함하지 않으며**, `get-model.ps1` 이 **공식 HuggingFace** 에서 내려받습니다(원본 배포처 준수).
...
- `llama-cpp-python`/llama.cpp(MIT), `customtkinter`(MIT/CC), `pystray`(LGPL/GPL 듀얼),
  `Pillow`(HPND), `keyboard`(MIT), `pyperclip`(BSD), `pywin32`(PSF).
  재배포 시 각 라이선스 고지를 따르세요.

> 요약: **기본 Gemma 2 2B 는 회사/상업용 OK**. 한국어 품질을 더 원하는 **개인용은 EXAONE(비상업)** 로 교체.
