...
class App:
    def __init__(self):
        ...
        # 단축키
        self.hotkeys = HotkeyManager(self.cfg.get("hotkeys", {}), self._on_hotkey)
    ...
    def _handle(self, mode: str):
        text = clipboard.capture_selection(
            restore=self._ui_cfg.get("restore_clipboard", True),
            timeout_ms=int(self._ui_cfg.get("capture_timeout_ms", 350)),
        )
        if not text:
            self.popup.root.after(0, lambda: self.popup.toast("선택된 텍스트가 없습니다.\n텍스트를 드래그한 뒤 다시 시도하세요."))
            return
        # 메인 스레드에서 팝업 시작
